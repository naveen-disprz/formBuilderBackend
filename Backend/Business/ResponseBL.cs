...
public class ResponseBL : IResponseBL
{
        ...
        Guid userId)
    {
        try
        {
            ...
            if (!form.IsPublished)
            {
                throw new InvalidOperationException("Cannot submit response to unpublished form");
            }
            ...
            if (hasResponded)
            {
                throw new InvalidOperationException("You have already responded to this form");
            }
    ...
    public async Task<ResponseListDto> GetFormResponsesAsync(string formId, int page, int pageSize, Guid userId)
    {
        try
        {
            ...
            {
                throw new UnauthorizedAccessException("You can only view responses for your own forms");
            }
    ...
    public async Task<ResponseDetailDto> GetResponseByIdAsync(Guid responseId, Guid userId, string userRole)
    {
        try
        {
            ...
            if (response == null)
            {
                throw new KeyNotFoundException($"Response not found: {responseId}");
            }
    ...
    public async Task<byte[]> GetFileContentAsync(Guid fileId, Guid userId, string userRole)
    {
        try
        {
            ...
            return Convert.FromBase64String(file.FileContent);
        }
        catch (Exception ex)
        {
